...
// Full HomePage implementation (copy-paste this entire file).
// NOTE: This version avoids importing dart:html so it can be built for desktop (Windows).
// Web-specific functionality (if any) should be placed in web-only files and conditionally imported.
...
import 'package:qr_sorter/pages/groups_page.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  // === Dialog that was requested by you (copied from your snippet) ===
  void _showItemDialog(Item item) {
    // Use a general dialog to avoid intrinsic sizing problems
    showGeneralDialog(
      context: context,
  ...
  Future<void> _printMultiple(List<Item> items, int copiesPerItem) async {
    try {
      // Create PDF and call Printing.layoutPdf (works on web and native).
      ...
      appBar: AppBar(
        ...
        actions: [
          if (_selectionMode) ...[
            ...
            // NEW: Groups button - opens grouped view (by category)
            IconButton(
              tooltip: 'Groups',
